...
class Membervoucher_model extends CI_Model
{
    ...
    // datatables
    function json() {
        $this->datatables->select('idx,idmember,idvoucher,idproduk,tglpakai,isdipakai,idjenisvoucher,idreveral,isdipakaireveral');
        $this->datatables->from('membervoucher');
        //add this line for join
        //$this->datatables->join('table2', 'membervoucher.field = table2.field');
        $this->datatables->add_column('action', anchor(site_url('membervoucher/read/$1'),'Read')." | ".anchor(site_url('membervoucher/update/$1'),'Update')." | ".anchor(site_url('membervoucher/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'idx');
