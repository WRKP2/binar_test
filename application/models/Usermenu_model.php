...
class Usermenu_model extends CI_Model
{
    ...
    // datatables
    function json() {
        $this->datatables->select('idx,iduser,idmenu,idaplikasi');
        $this->datatables->from('usermenu');
        //add this line for join
        //$this->datatables->join('table2', 'usermenu.field = table2.field');
        $this->datatables->add_column('action', anchor(site_url('usermenu/read/$1'),'Read')." | ".anchor(site_url('usermenu/update/$1'),'Update')." | ".anchor(site_url('usermenu/delete/$1'),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'idx');
