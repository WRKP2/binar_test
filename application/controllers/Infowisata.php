...
public function readinfowisataAndroid() {
		...
		$xQuery = $this->Infowisata_model->getListinfowisata();
...
public function simpanupdateinfowisataAndroid() {
        $this->load->helper('json'); 
...
$xidx = '0';
		...
		$xStr = $this->Infowisata_model->Updateinfowisata($xidx,$xjudulinfo,$xdeskripsihtml,$ximgutama,$xmapadress,$xtglinsert,$xtglupdate,$xidpegawai);
		} else {
            //===INSERT===
            
		$xStr = $this->Infowisata_model->Insertinfowisata($xidx,$xjudulinfo,$xdeskripsihtml,$ximgutama,$xmapadress,$xtglinsert,$xtglupdate,$xidpegawai);
            	}
            		}
...
public function deletinfowisataAndroid() {
        
		$xidx = $_POST['$edidx'];
        $this->load->model('Infowisata_model');
        $this->Infowisata_model->Deletinfowisata($xidx);
        $this->load->helper('json');
        echo json_encode(null);
    }
...
public function getDetailinfowisataAndroid() {
        
		$xidx = $_POST['$edidx'];
        $this->load->model('Infowisata_model');
        $this->Infowisata_model->getDetailinfowisata($xidx);
        $this->load->helper('json');
		$this->json_data['idx'] = $row->idx;
		$this->json_data['judulinfo'] = $row->judulinfo;
		$this->json_data['deskripsihtml'] = $row->deskripsihtml;
